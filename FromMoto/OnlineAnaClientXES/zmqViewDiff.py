...
class MainWindow(QtGui.QWidget):
    ...
    def UpdatePlot(self, port):
        if self.runCheck.isChecked():
            diffSignal = self.zeromqListener1.data - self.zeromqListener2.data
            self.plt.plot(diffSignal, pen='r', clear=True)
            if port == 5555:
                print("{0} - {1} = {2}".format(self.zeromqListener1.data.sum(), self.zeromqListener2.data.sum(), diffSignal.sum()))
                sys.stdout.flush()
